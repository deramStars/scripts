...
================Loon==============
[Script]
cron "50 * * * *" script-path=jd_dreamFactory.js,tag=京喜工厂
...
async function joinLeaderTuan() {
  let res = await updateTuanIdsCDN('http://193.112.28.110/inviteCode/jd_updateFactoryTuanId.json')
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `http://share.turinglabs.net/api/v8/jxfactory/query/${randomCount}/`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
