...
function submitCode() {
    return new Promise(async resolve => {
        $.get({
            url: `http://www.helpu.cf/jdcodes/submit.php?code=${myInviteCode}&type=ddfactory`,
            timeout: 10000
        }, (err, resp, data) => {
            try {
                if (err) {
...
function readShareCode() {
    ...
    return new Promise(async resolve => {
        $.get({
            url: "http://193.112.28.110/inviteCode/JD_Factory.json", headers: {
            ...
            try {
                if (err) {
                    ...
                    $.get({
                        url: `https://raw.githubusercontent.com/shuyeshuye/RandomShareCode/main/JD_Factory.json`,
                        'timeout': 10000
                    }, (err, resp, data) => {
                        data = JSON.parse(data);
                    })
